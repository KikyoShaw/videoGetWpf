...
namespace videoGet.ViewModel
{
    public class XiGuaGet
    {
        ...
        {
            if (url.Trim().Length == 29) GetHtml(url);
            //得到长链接
            GetHtml(url);
        }
        ...
        private void GetHtml(string url)
        {
            try
            {
                ...
                doc.LoadHtml(text);
                var str = doc.DocumentNode.SelectSingleNode("//body/script[@id='SSR_HYDRATED_DATA']").InnerText.Replace("window._SSR_HYDRATED_DATA=", "");
        ...
        public string FormatBase64(string str)
        {
            ...
            try
            {
                ...
                tempStr = Encoding.UTF8.GetString(array);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"WeiShiGet FormatBase64 is error: {ex}");
            }
            return tempStr;
        }
    }
